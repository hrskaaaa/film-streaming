...
paginationContainer.innerHTML = `
    ...
    <nav aria-label="TvShows pagination">
...
tvshowsGrid.parentNode.insertBefore(paginationContainer, loadMoreBtn.parentNode);
...
function updateResultsInfo() {
  ...
  function loadTVShows() {
    ...
    fetch(`/api/tvshows/?${params.toString()}`)
      ...
      .then((data) => {
        ...
        if (data.tvshows && data.tvshows.length > 0) {
          ...
          totalPages = data.total_pages || Math.ceil((data.count || data.tvshows.length) / 20);
          totalResults = data.count || data.tvshows.length;
    ...
    tvshows.forEach((show) => {
      ...
      col.innerHTML = `
            <div class="card-container card h-100 hover-shadow">
              <img 
                src="${show.poster_url || '/static/images/no-poster.jpg'}" 
                alt="${show.title}" 
              ...
              >
                ...
                <div class="card-body">
                    <h6 class="card-title mb-1">${show.title.length > 20 ? show.title.substring(0, 20) + '...' : show.title}</h6>
                    <div class="d-flex justify-content-between align-items-center small text-muted mb-2">
                        <span class="badge bg-info">TV Show</span>
                        <span>${show.release_date ? new Date(show.release_date).getFullYear() : 'N/A'}</span>
                    </div>
                    <div class="d-flex justify-content-between small text-muted">
                        ${show.rating ? `<span><i class="bi bi-star-fill text-warning"></i> ${parseFloat(show.rating).toFixed(1)}</span>` : ''}
                        ${show.runtime ? `<span>${Math.floor(show.runtime/60)}h ${show.runtime%60}m</span>` : ''}
                    </div>
                    <div class="mt-2">
                        <a href="/content/${show.id}/" class="btn btn-sm btn-outline-primary w-100">View Details</a>
                    </div>
                </div>
            </div>
        `;
        tvshowsGrid.appendChild(col);
    });
  }
});